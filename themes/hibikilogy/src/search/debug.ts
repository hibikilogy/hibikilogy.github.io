...
export function isSearchDebugEnabled(): boolean {
  try {
    return globalThis.localStorage?.getItem(searchDebugStorageKey) === '1'
  }
  catch {
    return false
  }
...
export function logSearchTiming(isEnabled: boolean, label: string, start: number): void {
  if (!isEnabled)
    return
  console.debug(`[search] ${label}: ${Math.round((nowMs() - start) * 10) / 10}ms`)
}
