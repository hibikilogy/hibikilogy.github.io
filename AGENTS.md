...
This repository is a Zola static site using the built-in `hibikilogy` theme. The theme lives in `themes/hibikilogy/` and contains templates, plain CSS (nested syntax, processed by lightningcss), static assets, TypeScript source (`src/`), and Web Components (`components/`).
...
- `styles/` — plain CSS stylesheets (nested syntax via lightningcss) organized as `base/`, `components/`, `layouts/`
- `static/` — theme static assets (JS bundle, CSS, SVG icons, fonts, KaTeX files)
- `src/` — TypeScript source organized as `app/` (lifecycle, composition), `features/` (search, page views), `ui/` (DOM adapters), `infrastructure/` (Swup integration, network), `shared/` (utilities, runtime config); compiled to `static/js/` by Vite. TS layering/review rules: `themes/hibikilogy/src/README.md`
...
`scripts/` holds the build-time tooling: a Rust crate (`scripts/rust/`, see the root `Cargo.toml`) plus TypeScript scripts. The Rust code runs only during builds — never in the site runtime. `scripts/README.md` covers the TS side; `scripts/rust/README.md` is the authoritative developer guide for the Rust crate.

- `scripts/beasties/` + `scripts/vite/` + `scripts/dev-cms/` + scripts `benchmark-search` / `verify-commit` / `checkI18nKeys.test.ts` — see `scripts/README.md`
- `scripts/rust/lib.rs` + `scripts/rust/shared/` — shared library modules (front matter, content files, URL encoding, managed file/JSON writes)
- `scripts/rust/bin/<tool>/` — five CLI binaries (`main.rs` + `app.rs` + domain modules):
  - `title-font-subset` / `body-font-subset` (feature `font-tools`) — subset fonts to glyphs actually used in content
  - `site-artifact-rewrite` (feature `artifact-rewrite`) — post-process `public/` per `scripts/rust/artifact-rewrite.toml` (URL cache busting, image attrs, thumbhash)
  - `article-short-links` — assign `/s/YYNNN/` short links and sync Zola `aliases` (ledger: `scripts/rust/data/short-link-reservations.json`)
...
- `cms/preview.css` — imports theme stylesheets (`critical.css`, `layouts/page.css`, `layouts/author-profile.css`) into the preview pane stylesheet.
...
- `pnpm build:rewrite-artifacts` — post-process `public/` per `scripts/rust/artifact-rewrite.toml`
- `pnpm sync:short-links` / `pnpm check:short-links` — assign or verify `/s/YYNNN/` short links against `scripts/rust/data/short-link-reservations.json`
