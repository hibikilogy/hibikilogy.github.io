use super::{collect_titles_from_content, extract_title, generate_title_font_subset, Args};
use crate::codepoints::collect_title_codepoints;
use hibikilogy_tools::font::coverage::collect_latin_codepoints;
use std::fs;
...
fn generate_options(content_dir: PathBuf, temp: &Path) -> Args {
    Args {
        content_dir,
        font: Path::new("themes/hibikilogy/static/fonts/SourceHanSerifCN-VF.ttf").to_path_buf(),
...
fn codepoints_split_into_latin_and_cjk_groups() {
    ...
    let latin = collect_latin_codepoints(["中文标题! A"]);
...
fn codepoints_skip_control_characters() {
    ...
    let latin = collect_latin_codepoints(["\u{0000}\u{0001}A"]);
