...
use super::{collect_body_text_from_content, collect_toml_string_fragments};
...
use hibikilogy_tools::font::coverage::{
    collect_cjk_codepoints, collect_latin_codepoints, font_codepoints,
};
...
fn body_subset_covers_cjk_including_kana_but_not_ascii() {
    let codepoints = collect_cjk_codepoints(
        &[
            "中文ABC".to_string(),
            "かなカナ".to_string(),
            "，。「」".to_string(),
...
fn latin_codepoints_cover_ascii_and_site_non_cjk() {
    let codepoints = collect_latin_codepoints(["中文ABC — é"]);
