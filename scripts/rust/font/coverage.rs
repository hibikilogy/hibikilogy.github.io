...
/// instead of the frequency chunks.
pub fn collect_latin_codepoints<I, S>(texts: I) -> Vec<u32>
where
...
/// CJK-family codepoints from `texts`, plus the `extra_retained` seeds (e.g.
/// CJK punctuation kept alongside the hanzi). These feed the
/// frequency-ordered chunk series.
pub fn collect_cjk_codepoints<I, S>(texts: I, extra_retained: &[u32]) -> Vec<u32>
where
    I: IntoIterator<Item = S>,
    S: AsRef<str>,
{
    collect_codepoints(texts, &[], extra_retained, is_cjk_codepoint)
}

/// Count per-codepoint occurrences of CJK characters across `texts`, feeding
/// the frequency-ordered chunking.
pub fn count_cjk_codepoints<I, S>(texts: I) -> std::collections::HashMap<u32, u64>
where
    I: IntoIterator<Item = S>,
    S: AsRef<str>,
{
