...
//!    slicing order (`scripts/data/font-slicing.config.json`, highest to
...
mod tests {
    ...
    fn model(site: &[(u32, u64)]) -> FrequencyModel {
        FrequencyModel::new(
            ...
            Path::new("scripts/data/font-slicing.config.json"),
        )
    ...
    fn google_layer_keeps_the_tables_relative_order() {
        ...
        let config: serde_json::Value = serde_json::from_str(
            &std::fs::read_to_string("scripts/data/font-slicing.config.json").unwrap(),
        )
        .unwrap();
